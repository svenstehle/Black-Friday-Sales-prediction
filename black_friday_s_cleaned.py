...
# the correct label encoding in test gave us over 200 RMSE decrease! Removing noise is important!
#%%
...
# and come from the same population (User and Product IDs are almost identical)
...
# distributions come from the sample population.
# But what does that mean for us? Does it even make sense to apply this test here in this way?
"""
#%%
...
# can't use that test here, right?
...
# pick the columns we want to create interaction terms for
...
def remove_correlated_variables(data, target, inplace = False):
    ...
    df_corr = data.drop(target, axis=1).corr(method='spearman')
    # create a mask to ignore self-correlation
    mask = np.ones(df_corr.columns.size) - np.eye(df_corr.columns.size)
    df_corr = mask * df_corr
    print("\t\nDone preparing correlation matrix")
    drops = []
    # loop through each variable
    for col in df_corr.columns.values:
        # if we've already determined to drop the current variable, continue
        if np.isin([col], drops):
            print("col {} already in drop list".format(col))
            continue
        # find all the variables that are highly correlated with the current variable
        # and add them to the drop list
        if np.any(df_corr[abs(df_corr[col]) > 0.95]):
            corr = df_corr[abs(df_corr[col]) > 0.95].index # try dropping at .95, 
            #some stats gurus say that it is almost guaranteed two variables are multicollinear above .9
            #besides we don't want to include variables that deliver almost
            #no extra information compared to another variable already in the dataset
            # therefore we can set at .95 and use feature selection / VIF removal later
            # to pick the best features and reduce the size of the dataset
            print("new drop is: {}".format(corr.values))
            drops = np.union1d(drops, corr)
        
    print("\nDropping", drops.shape[0], "highly correlated features...\n", drops)
    df = data.copy(deep=True) # avoid dropping the columns in the original df
    df.drop(drops, axis=1, inplace=inplace) # drop the columns only in copy df and return that to new df
    return df
#%%
...
import gc
del_list= [data1,df_interaction,data_val,dataset,test,train, scaled_data]
...
"""
scaled_data = remove_correlated_variables(scaled_data, target, inplace=True)
scaled_data.info()
#Dropping 96 highly correlated features...
#Columns: 196 entries, Occupation to Stay_In_Current_City_Years_code*Stay_In_Current_City_Years_code
#dtypes: float64(195), int64(1)

# with the min max scaler dataset we get:
#Dropping 162 highly correlated features...
#Columns: 130 entries, Occupation to City_Category_code/Stay_In_Current_City_Years_code
...
"""
# =============================================================================
# # regression stats - which variables have a high VIF and can be removed?
# # useful if we take the dummy dataset only? Test this. 
# Since we do a regression on every variable iteratively it might work better with dummies.
# =============================================================================
...
dataset = data1
#dataset = data1_dummy # dummy dataset with removed correlation variables. 

# Still multicollinearity in there?
...
y = dataset[target]
model = sm.OLS(y, X) # use this with dummies and linreg
results = model.fit()
print(results.summary())
# we get "significant" results for the non centered data without dummies
#%%
# let's see the difference for centered data
# centered data here (subtract mean)!
...
#gather features
#use statsmodels syntax 
#data = dataset
data = data1
features = "+".join(data[predictors].columns)
regression_on = target+" ~"

# get y and X dataframes based on this regression:
y, X = patsy.dmatrices(regression_on + features, data, return_type='dataframe')
#%%
...
scaled_data_vif = pd.concat([scaled_data_vif, scaled_data[target]], axis=1)
...
predictors = data.drop(target, axis=1).columns.values
data.info()
# from that dataset get dummies as well and see linear models reg RMSE score
"""
#%%
...
#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.2)
#2         40       2202   2526.99         30.9505       2402.6     1949
#1         30    2438.32    2627.6         34.0422       1458.4  1081.65
#0         20    2485.88   2646.46         30.4824       1459.2  838.566

#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.2)
#0         50    2198.53   2517.54         33.9358       2044.8  2110.72
#1        all    2173.27   2530.79         26.3182       1932.8  3847.96

#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.1)
#2         60    2151.09   2504.55         33.3351       4480.6  4526.72
#3         70    2130.57   2506.09         31.2074       4681.6  5359.62
#4         80    2118.63   2509.29          27.942       4723.8  6094.26
#5         90    2111.33   2512.24          28.857       4750.6   6811.4
#1         40    2159.62   2514.67         28.3531         5496  4063.03
#0         20     2471.8   2640.08         30.0307       3153.4  1529.26


### so for XGB we could do a different selection process making use of the feature importance scores instead of the f-statistic with select k best
# by selecting the most important features with an importance bigger than 5% of the most important features importance during splits we selected 52 features
# results from applying the selection process according to the cell below:
#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.1)
#0    all(52)    2118.23   2512.39         27.7772       4864.6  4586.98

#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.1)
#0         60    2118.33    2516.1         24.7675       5548.6  5754.69

# we did not use VIF selection process here
#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.3)
#9        all    2217.45   2534.39         24.0259         1177  3091.08
#8         90    2210.27   2541.01         25.5284         1297  2434.63
#6         70    2192.49   2542.09         22.8981       1439.4  2123.64
#7         80    2202.83   2543.06         27.3479       1366.6  2298.71
#5         60    2207.81    2547.3         24.0186       1455.8  1865.75
#4         50    2197.53   2563.82         22.5964       1658.4  1789.89
#2         30    2599.03   2811.17         30.3085        905.6  812.786
#3         40    2607.69   2813.33         37.9345          810  924.155
#1         20    2614.07   2813.54         36.0359        885.6  579.641
#0         10    2686.43   2865.55         28.8684       1329.8  504.637

#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.1)
#2        115    2127.39   2506.58         22.7594       4638.6  8222.81
#3        all    2133.91   2506.85         25.8207       4461.6   9115.3
#1         90    2113.93    2508.3         24.6726       5057.2  7236.81
#0         60    2118.33    2516.1         24.7675       5548.6   5606.8
# ok these are many features... lets check around 115 to find the best feature set size and then tune hyperparameters
#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.3)
#1        110    2207.87   2535.63         28.3991       1268.8  2912.48
#2        115    2226.74   2535.78          30.156       1162.2  2780.56
#3        120    2224.45   2536.71         25.2648         1156     2918
#0        100    2230.44   2537.45         26.8984       1162.4  2515.33
# ok we pick 110 features and go with tuning from there - did not apply VIF removal on features before
# first though we try out applying PCA below - PCA delivered bad results. Not using PCA on that dataset

# using VIF removal on features results in 19 predictors left and we test this now with select k best:
#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time (eta 0.3)
#0         10    2231.82   2527.07          19.175       1884.4  651.756
#1         13    2229.72    2527.1         21.8978       1854.4  772.534
#3        all    2190.24   2536.71         20.7117         1697  797.731
#2         16     2198.3   2537.37         24.7102       1804.2  790.252
...
#compare with results on data1 dataset without interaction terms... wow much better.
#We can keep our original results and go with the submissions for the full dataset
#  selected k RMSE train RMSE test RMSE test 3*STD boost_rounds     time
#0         10     2216.6   2489.75          25.418       1943.4  571.837
#1        all    2226.53   2490.29         25.0938         1837   570.81
...
# # simple process without selection by CV score... 
# # selection by CV score below
# =============================================================================
data = scaled_data
predictors = scaled_data.drop(target, axis=1).columns.values
...
important_features = np.array([feature_importance.keys()[x] for x in important_idx])
...
plt.subplot(1, 2, 2)
plt.barh(pos, feature_importance.loc[sorted_idx.index.values], align='center')
plt.yticks(pos, sorted_idx.index)
...
for k in [2,5,10,15,20,30]: 
    ...
    sorted_idx = np.argsort(feature_importance[important_idx])[::-1]
    print("\nFeatures sorted by importance (DESC):\n", important_features[sorted_idx])
    # set up lists for results in CV loop
    train_score = []
    val_score = []
    boost_rounds = []
    #define new train set for CV loop with reduced columns according to feature importance threshold
    for n, (train_i, test_i) in enumerate(cv_split.split(data[important_features[sorted_idx]], data[target])): 
        # use kfold and "average" over the whole dataset, use early stopping in xgboost.train for every eval_set
        print("\nFitting CV folds k = {}...".format(n+1))    
        X_train, X_val = data[important_features[sorted_idx]].iloc[train_i], data[important_features[sorted_idx]].iloc[test_i]
        y_train, y_val= data[target].iloc[train_i], data[target].iloc[test_i]
                
        print("CV with selected threshold of {}  now".format(k))
        
        #DMatrix for every train and val set in folds
        dtrain = xgboost.DMatrix(X_train, label=y_train.values, feature_names = important_features[sorted_idx], nthread = 8)
        dtest = xgboost.DMatrix(X_val, label=y_val.values, feature_names = important_features[sorted_idx], nthread = 8)
    ...
    CVCompare.loc[row_index, "CV RMSE train"] = train_score_CV
    CVCompare.loc[row_index, "CV RMSE test"] = val_score_CV
    CVCompare.loc[row_index, "CV RMSE test 3*STD"] = val_acc_std*3
...
#3          20         2267.01        2497.46       2338.01      2489.29   
#4          29         2267.01        2497.46       2338.01      2489.29   
#2          15         2267.01        2497.46       2340.04      2490.66   
#1          10         2267.01        2497.46       2271.46      2493.78   
#0           5         2267.01        2497.46       2277.68      2500.71   
#
#  CV RMSE test 3*STD CV boost_rounds     time  
#3            22.4952            9705  1833.28  
#4            22.4952            9705  1878.86  
#2             23.148          9440.4  1785.04  
#1            22.5055          9495.2  2339.03  
#0            21.5347            7823  2461.75 

#k = 29:  ['Product_ID_code', 'Occupation/Product_Category_1', 'Occupation/Age_code',
# 'Occupation/Stay_In_Current_City_Years_code',
# 'Marital_Status/Product_Category_1', 'Marital_Status/User_ID_code']
# and we have less variance / overfitting on the train set 
# test scores for adding the original predictors to the set and cycle through the threshold to see if we can get a better value
"""
#%%
      
val_accuracy_base = 2514.21 
train_accuracy_base = 2165.32  
# for future tests so we dont need to run the first half of the code
pd.set_option("display.max_columns",22)
pd.set_option("display.max_rows",1500)
#len(dict(feature_importance))
# xgb kept 157 features, so we had about half with zero importance = not used in splitting at all

#%%

feature_importance_man = {'Age_code': 1.1173184357541899,
 'Age_code*City_Category_code': 1.8531135032020709,
 'Age_code*Stay_In_Current_City_Years_code': 1.8803651723668073,
 'Age_code+City_Category_code': 0.27251669164736342,
 'Age_code+Stay_In_Current_City_Years_code': 0.61316255620656768,
 'Age_code-City_Category_code': 0.49053004496525415,
 'Age_code-Stay_In_Current_City_Years_code': 0.89930508243629914,
 'Age_code/City_Category_code': 1.580596811554708,
 'Age_code/Stay_In_Current_City_Years_code': 2.0711268565199616,
 'City_Category_code': 0.55865921787709494,
 'City_Category_code*Stay_In_Current_City_Years_code': 1.5942226461370759,
 'City_Category_code+Stay_In_Current_City_Years_code': 0.13625834582368171,
 'City_Category_code-Stay_In_Current_City_Years_code': 0.29976836081209973,
 'City_Category_code/Stay_In_Current_City_Years_code': 1.2399509469955035,
 'Gender_code': 0.36789753372394057,
 'Gender_code*Age_code': 1.1036926011718218,
 'Gender_code*City_Category_code': 0.73579506744788115,
 'Gender_code*Stay_In_Current_City_Years_code': 0.64041422537130399,
 'Gender_code/Age_code': 1.362583458236817,
 'Gender_code/City_Category_code': 0.53140754871235862,
 'Gender_code/Stay_In_Current_City_Years_code': 0.76304673661261757,
 'Marital_Status': 0.98106008993050831,
 'Marital_Status*Age_code': 0.70854339828314483,
 'Marital_Status*City_Category_code': 0.66766589453604031,
 'Marital_Status*Gender_code': 0.19076168415315439,
 'Marital_Status*Product_Category_1': 2.7524185856383703,
 'Marital_Status*Product_Category_2': 1.9757460144433847,
 'Marital_Status*Product_Category_3': 1.5942226461370759,
 'Marital_Status*Stay_In_Current_City_Years_code': 1.0628150974247172,
 'Marital_Status*User_ID_code': 7.5078348548848624,
 'Marital_Status/Age_code': 1.7986101648725983,
 'Marital_Status/City_Category_code': 0.81755007494209031,
 'Marital_Status/Gender_code': 0.29976836081209973,
 'Marital_Status/Product_Category_1': 2.4935277285733752,
 'Marital_Status/Product_Category_2': 1.93486851069628,
 'Marital_Status/Product_Category_3': 1.4579643003133942,
 'Marital_Status/Stay_In_Current_City_Years_code': 0.9946859245128763,
 'Marital_Status/User_ID_code': 8.0801199073443239,
 'Occupation': 5.4912113366943727,
 'Occupation*Age_code': 3.0658127810328382,
 'Occupation*City_Category_code': 2.7524185856383703,
 'Occupation*Gender_code': 2.0029976836081209,
 'Occupation*Marital_Status': 3.1066902847799427,
 'Occupation*Product_Category_1': 5.6138438479356862,
 'Occupation*Product_Category_2': 4.9734296225643817,
 'Occupation*Product_Category_3': 4.1967570513693966,
 'Occupation*Stay_In_Current_City_Years_code': 2.9431802697915246,
 'Occupation*User_ID_code': 8.3798882681564244,
 'Occupation+Age_code': 2.2073852023436435,
 'Occupation+City_Category_code': 0.73579506744788115,
 'Occupation+Product_Category_1': 3.3792069764273061,
 'Occupation+Product_Category_2': 2.5071535631557436,
 'Occupation+Product_Category_3': 1.3489576236544489,
 'Occupation+Stay_In_Current_City_Years_code': 1.3762092928191851,
 'Occupation+User_ID_code': 6.3768905845483035,
 'Occupation-Age_code': 2.0438751873552254,
 'Occupation-City_Category_code': 1.4715901348957623,
 'Occupation-Product_Category_1': 3.4337103147567785,
 'Occupation-Product_Category_2': 2.9976836081209974,
 'Occupation-Product_Category_3': 1.7713584957078623,
 'Occupation-Stay_In_Current_City_Years_code': 1.6214743153018123,
 'Occupation-User_ID_code': 7.0990598174138162,
 'Occupation/Age_code': 2.6979152473088979,
 'Occupation/City_Category_code': 2.4935277285733752,
 'Occupation/Gender_code': 2.7524185856383703,
 'Occupation/Marital_Status': 2.7251669164736341,
 'Occupation/Product_Category_1': 5.9681155470772591,
 'Occupation/Product_Category_2': 4.1695053822046599,
 'Occupation/Product_Category_3': 3.8424853522278237,
 'Occupation/Stay_In_Current_City_Years_code': 2.8069219239678431,
 'Occupation/User_ID_code': 7.37157650906118,
 'Product_Category_1': 8.148249080256166,
 'Product_Category_1*Age_code': 3.6653495026570377,
 'Product_Category_1*City_Category_code': 2.5752827360675843,
 'Product_Category_1*Gender_code': 2.6979152473088979,
 'Product_Category_1*Product_Category_2': 7.5759640277967026,
 'Product_Category_1*Product_Category_3': 6.336013080801199,
 'Product_Category_1*Stay_In_Current_City_Years_code': 2.8341735931325793,
 'Product_Category_1*User_ID_code': 17.086796566289685,
 'Product_Category_1+Age_code': 2.1120043602670666,
 'Product_Category_1+City_Category_code': 0.70854339828314483,
 'Product_Category_1+Product_Category_2': 2.9159286006267884,
 'Product_Category_1+Product_Category_3': 1.6351001498841806,
 'Product_Category_1+Stay_In_Current_City_Years_code': 1.7304809919607576,
 'Product_Category_1+User_ID_code': 7.548712358631966,
 'Product_Category_1-Age_code': 2.3300177135849571,
 'Product_Category_1-City_Category_code': 1.3353317890720806,
 'Product_Category_1-Product_Category_2': 3.5563428259980925,
 'Product_Category_1-Product_Category_3': 1.130944270336558,
 'Product_Category_1-Stay_In_Current_City_Years_code': 1.8667393377844392,
 'Product_Category_1-User_ID_code': 7.9302357269382755,
 'Product_Category_1/Age_code': 4.0604987055457151,
 'Product_Category_1/City_Category_code': 2.9023027660444201,
 'Product_Category_1/Gender_code': 2.9840577735386296,
 'Product_Category_1/Product_Category_2': 7.3034473361493397,
 'Product_Category_1/Product_Category_3': 3.9514920288867694,
 'Product_Category_1/Stay_In_Current_City_Years_code': 3.161193623109416,
 'Product_Category_1/User_ID_code': 16.160239814688648,
 'Product_Category_2': 5.0006812917291183,
 'Product_Category_2*Age_code': 2.8886769314620522,
 'Product_Category_2*City_Category_code': 2.0302493527728576,
 'Product_Category_2*Gender_code': 1.7986101648725983,
 'Product_Category_2*Product_Category_3': 5.6274696825180541,
 'Product_Category_2*Stay_In_Current_City_Years_code': 2.3163918790025892,
 'Product_Category_2*User_ID_code': 9.9196075759640276,
 'Product_Category_2+Age_code': 0.92655675160103546,
 'Product_Category_2+City_Category_code': 0.51778171412999041,
 'Product_Category_2+Product_Category_3': 1.5533451423899713,
 'Product_Category_2+Stay_In_Current_City_Years_code': 1.212699277830767,
 'Product_Category_2+User_ID_code': 5.981741381659627,
 'Product_Category_2-Age_code': 1.1854476086660308,
 'Product_Category_2-City_Category_code': 0.59953672162419946,
 'Product_Category_2-Product_Category_3': 1.7986101648725983,
 'Product_Category_2-Stay_In_Current_City_Years_code': 1.2944542853249763,
 'Product_Category_2-User_ID_code': 6.1997547349775175,
 'Product_Category_2/Age_code': 3.5699686605804604,
 'Product_Category_2/City_Category_code': 2.0711268565199616,
 'Product_Category_2/Gender_code': 2.2482627060907481,
 'Product_Category_2/Product_Category_3': 7.4533315165553891,
 'Product_Category_2/Stay_In_Current_City_Years_code': 2.5344052323204793,
 'Product_Category_2/User_ID_code': 10.342008448017442,
 'Product_Category_3': 3.1339419539446793,
 'Product_Category_3*Age_code': 2.1120043602670666,
 'Product_Category_3*City_Category_code': 1.4034609619839216,
 'Product_Category_3*Gender_code': 1.3080801199073444,
 'Product_Category_3*Stay_In_Current_City_Years_code': 1.9484943452786483,
 'Product_Category_3*User_ID_code': 8.4752691102330022,
 'Product_Category_3+Age_code': 0.40877503747104516,
 'Product_Category_3+City_Category_code': 0.23163918790025886,
 'Product_Category_3+Stay_In_Current_City_Years_code': 0.62678839078893589,
 'Product_Category_3+User_ID_code': 4.5237770813462319,
 'Product_Category_3-Age_code': 0.76304673661261757,
 'Product_Category_3-City_Category_code': 0.27251669164736342,
 'Product_Category_3-Stay_In_Current_City_Years_code': 0.51778171412999041,
 'Product_Category_3-User_ID_code': 5.4230821637825315,
 'Product_Category_3/Age_code': 3.0658127810328382,
 'Product_Category_3/City_Category_code': 1.8531135032020709,
 'Product_Category_3/Gender_code': 1.907616841531544,
 'Product_Category_3/Stay_In_Current_City_Years_code': 2.1392560294318028,
 'Product_Category_3/User_ID_code': 9.5653358768224539,
 'Product_ID_code': 100.0,
 'Stay_In_Current_City_Years_code': 0.98106008993050831,
 'User_ID_code': 7.6985965390380162,
 'User_ID_code*Age_code': 8.1209974110914303,
 'User_ID_code*City_Category_code': 8.4616432756506335,
 'User_ID_code*Gender_code': 6.9764273061725026,
 'User_ID_code*Stay_In_Current_City_Years_code': 7.6713448698732796,
 'User_ID_code+Age_code': 5.9681155470772591,
 'User_ID_code+City_Category_code': 2.8614252622973155,
 'User_ID_code+Stay_In_Current_City_Years_code': 4.3466412317754468,
 'User_ID_code-Age_code': 6.4995230957896162,
 'User_ID_code-City_Category_code': 3.515465322250988,
 'User_ID_code-Stay_In_Current_City_Years_code': 4.9598037879820138,
 'User_ID_code/Age_code': 7.3443248398964434,
 'User_ID_code/City_Category_code': 7.8893582231911701,
 'User_ID_code/Gender_code': 6.7039106145251397,
 'User_ID_code/Stay_In_Current_City_Years_code': 6.7584139528546121}
#%%
feature_importance=pd.Series(feature_importance_man)
sorted(feature_importance)
#%%
[k/10 for k in range(50,60)]
#%%
#k: A threshold below which to drop features from the final data set. 
# the percentage of the most important feature's importance value
# Can cycle through threshold with CV
...
for k in [30, 110]: #values for PCA
    #set up lists for results
    train_score = []
    val_score = []
    boost_rounds = []
...
"""
# =============================================================================
# get dummies for linear models, they deliver bad results with the encoded categorical columns 
# =============================================================================
#%%
# First: get dummies from our LabelEncoded columns. # we haven't so far
dummy_code = ['Occupation', 'Marital_Status', 'Product_Category_1', 'Product_Category_2',
 'Product_Category_3', 'Gender_code',
 'Age_code', 'City_Category_code', 'Stay_In_Current_City_Years_code']
data1_dummy = pd.get_dummies(data = data1[dummy_code], columns=dummy_code, drop_first=True)
data1_dummy = pd.concat([data1_dummy, data1[["User_ID_code", "Product_ID_code", target]]], axis=1)
#%%
# dummies for full dataset with userid and product id
dummy_code = ['User_ID_code', 'Product_ID_code', 'Occupation', 'Marital_Status', 'Product_Category_1', 'Product_Category_2',
 'Product_Category_3', 'Gender_code',
 'Age_code', 'City_Category_code', 'Stay_In_Current_City_Years_code']
data1_dummy = pd.get_dummies(data = data1[dummy_code], columns=dummy_code, drop_first=True)
...
#%%
# dummies for dataset with interaction variables + userid and product id

data1_dummy = pd.get_dummies(data = scaled_data.drop([target], axis = 1), columns=scaled_data.drop([target], axis = 1).columns, drop_first=True)
data1_dummy = pd.concat([data1_dummy, scaled_data[target]], axis=1)
  
# use this for linear regression and try it out :)
"""
#%%
...
for name, alg in MLA:
    ...
    cv_results = model_selection.cross_validate(alg, data1[predictors], data[target], cv=cv_split, scoring = "neg_mean_squared_error", return_train_score=True, n_jobs = -1, verbose = 10)
...
#we'll take chance here and don't explore this any further
"""
        
#%%

# =============================================================================
# # learning curves for different algorithms tested by hand
# =============================================================================

#predictors = data1_code
#data = data1 # imputed 0's for missing values in categories
data = df
...
#predictors = df_interaction.drop(target, axis=1).columns
#data = df_interaction

#neural network test
#model=MLPRegressor(hidden_layer_sizes=(100, ), activation='relu', solver='adam', alpha=0.0001, batch_size='auto', learning_rate='constant', 
#                   learning_rate_init=0.0003, power_t=0.5, max_iter=900, shuffle=True, random_state=42, tol=0.0001, verbose=False, 
#                   warm_start=False, momentum=0.9, early_stopping=True) # low var high bias without dummies...
...
model = XGBRegressor(n_estimators = 1000, n_jobs = 8, random_state = 42) # good! # 3186 rmse at 0.05 tr/te .... data1_dummy: 3544
#model = SVR() # takes a loooong time to train svr on that amount of data
#model = XGBRegressor(n_estimators = 1000, seed = 42, random_state = 42) # good!
#model = LinearRegression() # rmse for data1: 4613 / rmse for data1_dummy: 2976!!! at least our linearregression fares well with the dummies and less collinearity
# rmse for data1_dummy with all dummies (10+ minutes to create the dummies)
#model = ElasticNet() # rmse for data1: 4617 / rmse for data1_dummy: 4623
#model = RANSACRegressor(min_samples=10) # rmse for data1: 5645 / rmse for data1_dummy: 4414
#model = ElasticNetCV() # OK
#model = linear_model.Lasso(max_iter=2000) # OK # rmse for data1: 4613 / rmse for data1_dummy: 2978
#model = linear_model.LassoCV(max_iter=2000) # OK # rmse for data1:  / rmse for data1_dummy: 
#model = linear_model.Ridge() # OK # rmse for data1:  / rmse for data1_dummy: 2976

#model = neighbors.KNeighborsRegressor() # ok needs tuning
plot_learning_curves_CV_reg(model, data, predictors, target, suptitle = "learning curve tests", title="base model "+str(model.__class__.__name__)+" CV scores", xlabel="n samples", ylabel="RMSE", ylabel2="r2 score", k = 5, granularity = 5, train_size = 0.9, test_size = 0.1,  scale = False, center = False, split = "KFold", random_state = 42)

# We only learned too late that making dummies and using some linear models might have helped us in later stacking or other experiments. 
# we created some interaction variables and took a look at CV results after centering the data and removing collinear variables
#%%

# =============================================================================
#  get the cv performance of different variable sets
#  compare removing nan columns CV RMSE with full variable set
# =============================================================================
...
data = data1
#predictors = data1_code
predictors_remove = ['User_ID_code', 'Product_ID_code', 'Occupation', 'Marital_Status', 'Product_Category_1', 
  'Gender_code',
 'Age_code', 'City_Category_code', 'Stay_In_Current_City_Years_code'] # no nan columns
...
train_rmse, test_rmse, train_r2, test_r2 = CV_target_max_purchase(model, data, predictors_remove, target, k = 5)
...
#    print("Result for ", result, " is: ", value)

# XGB : CV RMSE for all features, no dummies
#                RMSE
#Train RMSE  2698.52
#Test RMSE   2709.67
#Train r2       0.71
...
# linreg no dummies RMSE
#Train RMSE  4613.33
#Test RMSE   4613.43
#Train r2       0.16
#Test r2        0.16

#xgb results for removed columns with nan --> worse results
#               RMSE
#Train RMSE  2705.83
#Test RMSE   2717.14
#Train r2       0.71
...
# Test out effects of tinkering with a max_purchase value ceiling for Purchase. Removing or changing the values will be explored. 
# Although I guess changing the value of the train data itself is bad practice. I saw this in one submission on github and I want to do CV on it anyway to look at the results.
...
#%%
# cv results for removing the max samples from the set altogether:
data = data1
predictors = data1_code
target = "Purchase"
...
#        y_predict = estimator.predict(X)
#    return np.sqrt(metrics.mean_squared_error(y, y_predict))
#
...
#%%
# import graphviz <-- need to install 
#plt.figure(figsize=(20,15))
#xgboost.plot_tree(model, ax=plt.gca())
#%%
...
params = {
        # parameters for original dataset tuned further...
    'max_depth':4,
    'min_child_weight': 13,
    'eta': 0.1,
    'subsample': 1,
...
# second tuning round (eta 0.2,  early stopping 400)
#%%
...
# Should loop this for cv result on the whole train set (not just part as dtest)
#%%
"""
# use number of trees for fitting the model ... but can also use best_ntree_limit for prediction
...
# test on val set
print("RMSE on test set: {:.2f}".format(np.sqrt(mean_squared_error(y_test,best_model.predict(dtest)))))
# RMSE on test set: 2472.72
"""
#%%
...
#loaded_model.load_model("my_ model.model")
...
params = {
    ...
    # Other parameters
    'objective':'reg:linear',
    #'booster':'gblinear', # instead of gbtree for testing? #test this with dummies as well!
    # for gblinear we get horrible results of 4716.56 RMSE as we did for lin reg directly. Are dummies better?
    'alpha': 0.0,
    'lambda': 30.,
    'gamma': 0.,
    'seed' : 42,
...
# employ an ANN
# could use NMF on dummies dataset and see if the interactions are more easily accessible to the estimator...
